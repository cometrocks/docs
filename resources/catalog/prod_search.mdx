...
To retrieve products in manageable chunks, use the pagination parameters. If pagination is not provided, the default page size is 10 records. For more details on pagination, refer to the [Pagination Documentation](#link-to-pagination-documentation).

### Product Results

The `productFind` query returns a `ProductConnection` type, which includes:

- **pageInfo**: Information about the current page of results.
- **nodes**: A list of products that match the search criteria.
